...
第二要素の現在の誤差 $E = x'^2a^{-1}$ は $\mathop{\mathrm{ind}}E = (0, w)$ と表される。$\ZZ/2^Q\ZZ$ の元を $Q$ 桁の2進数と考えると、まず $w$ の $S$ 桁目が $1$ であるか判定し、そうであれば $x'$ に $S-1$ 桁目が $1$ である数を足して $S$ 桁目を $0$ にする。

最下位ビットが $1$ である

ここである非平方剰余な数 $b$ ($b^{(p-1)/2} = 1$ を満たす $b$ ) を選ぶと、$\mathop{\mathrm{ind}}b = (b_1, b_2)$ と表される。すると $q$ 乗して $\mathop{\mathrm{ind}}b^q = (0, b')$ と表される。$b$ は非平方剰余であるから $b_2$ は奇数であり、$b'$ も奇数である。
$\ZZ/2^Q\ZZ$ を $Q$ 桁の2進数として見ると、$(\ZZ/p\ZZ)^\times$ での2乗と $\ZZ/2^Q\ZZ$ での左シフトを対応させられることが分かる。
これより $b'$ の最下位ビットは $1$ である。


$\ZZ/2^Q\ZZ$ の視点で考える。
まず $w$ の $S$ 桁目が下から見て最初の $1$ であるか判定し。$w$ を $Q - S - 1$ 回左シフトした数が $0$ だったら $S$ 桁目は $0$ であり、そうでなければ $1$ である。
$w$ の $S$ 桁目が下から見て最初の $1$ であるなら $b'$ を $S-1$ 回左シフトしたものを $\mathop{\mathrm{ind}}x'$ の第二要素に足す。

$(\ZZ/p\ZZ)^\times$ の視点で考える。
つまり $E^{2^{Q-S-1}} \neq 1$ であったら新しい $x'$ を $x'' = x'(b^q)^{2^{S-1}}$ とする。
